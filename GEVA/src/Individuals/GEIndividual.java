 ...
 * Only has one chromosome and grammar, except now I am trying to make that two chromosomes
 * @author Blip
 */
...
public class GEIndividual extends AbstractIndividual {
	...
	this.previouslyValid = false;
        ...
        /**
     * Create new D-GEindividual instance
     * @param g mapper(grammar)
     * @param p output(phenotype)
     * @param gen input(genotype)
     * @param gen_b input(genotype)
     * @param f fitness
     */
    public GEIndividual(GEGrammar g, Phenotype p, Genotype gen, Genotype gen_b, Fitness f) {
        this.grammar = GEGrammar.getGrammar(g);
        this.phenotype = p;
        this.genotype = gen;
        this.genotype_b = gen_b;
        this.fitness = f;
        this.evaluated = false;
        this.mapped = false;
        this.valid = false;
	this.previouslyValid = false;
        ...
        this.grammar.setPhenotype(this.phenotype);
        this.grammar.setGenotype(this.genotype.get(0));
        this.grammar.setGenotype(this.genotype_b.get(0));
        this.fitness.setIndividual(this);
        this.age = 0;
    ...
    private GEIndividual(GEIndividual i) {
        ...
        if (i.getGenotype_b() != null) {
            this.genotype_b = new Genotype(1);
            this.genotype_b.add(new GEChromosome((GEChromosome) i.getGenotype_b().get(0)));
            this.grammar.setGenotype(this.genotype_b.get(0));
        }
    ...
    /**
     * Factory method for creating GEIndividual with different grammars
     * @param grammar grammar
     * @param phen phenotype
     * @param genotype genotype
     * @param genotype_b genotype
     * @param fitness fitness
     * @return new GEIndividual
     */
    public static GEIndividual getIndividual(final GEGrammar grammar, final Phenotype phen, final Genotype genotype, final Genotype genotype_b, final Fitness fitness) {
        final GEIndividual gei;
	gei = new GEIndividual(grammar, phen, genotype, genotype_b, fitness);
        return gei;
    }
    ...
    public Genotype getGenotype_b() {
        return this.genotype_b;
    }
     ...
     * Sets the new genotype in the individual as well as in the mapper
     * @param g genotype
     */
    ...
    public void setGenotype_b(Genotype g) {
        this.invalidate();
        this.genotype_b = g;
        this.getMapper().setGenotype(this.genotype_b.get(0));
    }
    ...
    /**
     * Clone this individual, invalidate and return the clone
    ...
    /**
     * Get number of codons used for mapping previously. Used when the
